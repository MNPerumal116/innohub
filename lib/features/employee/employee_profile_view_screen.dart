...
import '../../routes/app_routes.dart'; // assuming we need routing for edit later
...
class _EmployeeProfileViewScreenState extends State<EmployeeProfileViewScreen>
  ...
  // Keep a local copy of employee that can be updated from bloc
  ...
  void initState() {
    ...
    context.read<EmployeeDetailBloc>().add(FetchEmployeeDetail(_employee.id));
  }
  ...
  Employee get emp => _employee;
  String get initials => emp.displayName.isNotEmpty ? emp.displayName[0] : 'U';
    ...
    return BlocConsumer<EmployeeDetailBloc, EmployeeDetailState>(
      listener: (context, state) {
        if (state is EmployeeActionSuccess) {
          ...
          if (state.updatedEmployee != null) {
            setState(() => _employee = state.updatedEmployee!);
          }
        } else if (state is EmployeeDetailLoaded) {
          setState(() => _employee = state.employee);
          ...
          SliverAppBar(
            ...
            actions: [
              IconButton(
                ...
                onPressed: () {
                  Navigator.pushNamed(context, AppRoutes.editEmployee, arguments: _employee);
                },
              ),
              PopupMenuButton<String>(
                ...
                onSelected: (value) {
                  if (value == 'deactivate') {
                    context.read<EmployeeDetailBloc>().add(DeactivateEmployee(_employee.id));
                  }
                },
                itemBuilder: (context) => [
                  const PopupMenuItem(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildInfoRow(
                        ...
                        'Manager',
                        widget.employee.managerId?.toString() ?? 'N/A',
                      ),
                  ...
                  Row(
                    children: [
                      ...
                      _metaBlock('MANAGER', emp.managerId?.toString() ?? 'N/A'),
                    ],
                  ),
                ],
              ),
